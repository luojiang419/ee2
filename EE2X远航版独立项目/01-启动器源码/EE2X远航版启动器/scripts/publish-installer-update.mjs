...
async function main() {
  ...
  const baseUrl = String(
    args["base-url"] || process.env.EE2X_YH_LAUNCHER_UPDATE_BASE_URL || "http://115.231.35.105:3015"
  ).replace(/\/+$/, "");
  const username = String(
    args.username || process.env.EE2X_YH_LAUNCHER_UPDATE_USERNAME || "ee2x"
  );
  const password = String(
    args.password || process.env.EE2X_YH_LAUNCHER_UPDATE_PASSWORD || "ee2x"
  );
