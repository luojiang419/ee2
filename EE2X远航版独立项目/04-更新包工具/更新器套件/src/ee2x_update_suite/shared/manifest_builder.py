...
def _validate_scope_selection(
    ...
    """验证选择范围，运行时敏感目录已被 BLOCKED_SELECTION_PREFIXES 保护。"""
    validation = ValidationResult()
    all_paths = [*selected_relative_paths, *delete_list]
    has_launcher = any(classify_relative_path(item) == PACKAGE_SCOPE_LAUNCHER for item in all_paths)
    has_game = any(classify_relative_path(item) == PACKAGE_SCOPE_GAME for item in all_paths)
    if has_launcher and not has_game:
        validation.issues.append(
            ValidationIssue(
                level="warning",
                code="launcher-only-selection",
                message="当前仅选择了启动器文件，未包含游戏文件。启动器更新会在用户下次启动时通过独立更新进程覆盖。",
            )
        )
    return validation
...
def create_dual_release_bundle(
    *,
    ...
    validation = _validate_scope_selection(selected_relative_paths, delete_list)
    ...
    if not final_selected_relative_paths and not [item for item in delete_list if normalize_relpath(item)]:
        validation.issues.append(
            ValidationIssue(
                ...
                message="自动排除启动器本地状态文件后，当前没有可导出内容。请选择实际需要发布的文件。",
            )
        )
    if validation.has_errors:
        return output_root, {}, validation
