...
UP_RUNTIME_FROZEN_PREFIXES: dict[str, list[str]] = {
    PACKAGE_SCOPE_GAME: [],
    PACKAGE_SCOPE_LAUNCHER: [],
}
...
def _scope_needs_update(current_state: dict, latest: LatestRelease, scope: str, force: bool = False) -> tuple[str, ReleasePackage | None]:
    ...
    try:
        ...
        for index, file_entry in enumerate(manifest.files, start=1):
            ...
            if existed:
                ...
                # 先删除旧文件，再复制新文件，避免 rename 覆盖冲突
                try:
                    _remove_any(target_path)
                except Exception as del_err:
                    _append_log(log_file, f"[{scope}:删除旧文件警告] {rel_path}: {del_err}")
            target_path.parent.mkdir(parents=True, exist_ok=True)
            shutil.copy2(source_path, target_path)
            # 显式更新文件时间为当前时刻，避免 Windows 上保留 ZIP 内旧时间戳
            now = time.time()
