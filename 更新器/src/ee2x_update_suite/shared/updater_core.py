...
def _write_result_payload(result_file: Path | None, payload: dict) -> None:
    if result_file is None:
        return
    try:
        save_json(result_file, payload)
    except Exception:
        pass


def _success_result_payload(summary: ApplySummary, log_file: Path | None = None) -> dict:
    payload = {
        "ok": True,
        "scope": summary.scope,
