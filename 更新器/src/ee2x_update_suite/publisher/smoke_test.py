...
def main() -> None:
    ...
    try:
        ...
        try:
            ...
            (patcher_root / "UP15.dll").write_text("old-up15\n", encoding="utf-8")
            patcher_server_root = workspace / "patcher-apply-server"
            patcher_channel = "apply"
            ...
            _write_zip_with_root(
                ...
                ROOT_DIR_NAME,
                {
                    ...
                    "UnofficialVersionConfig.txt": b"new-config\n",
                    "UP15.dll": b"new-up15\n",
                },
            )
            patcher_manifest = {
                ...
                "files": [
                    ...
                    },
                    {
                        ...
                        "size": len(b"new-config\n"),
                        "sha256": hashlib.sha256(b"new-config\n").hexdigest(),
                    },
                    {
                        "path": "UP15.dll",
                        "size": len(b"new-up15\n"),
                        "sha256": hashlib.sha256(b"new-up15\n").hexdigest(),
                    },
                ],
            }
            ...
            _assert((patcher_root / "live_patch.txt").read_text(encoding="utf-8").strip() == "patched", "patcher 应正常应用补丁文件")
            _assert((patcher_root / "UnofficialVersionConfig.txt").read_text(encoding="utf-8").strip() == "new-config", "patcher 应正常覆盖原冻结区配置文件")
            _assert((patcher_root / "UP15.dll").read_text(encoding="utf-8").strip() == "new-up15", "patcher 应正常覆盖原冻结区 DLL")
            _assert(patcher_summary.updatedFiles == 3, "patcher 应更新全部 3 个文件")
