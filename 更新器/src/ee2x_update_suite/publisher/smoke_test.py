...
from ee2x_update_suite.shared.constants import LAUNCHER_DIR_NAME, PACKAGE_SCOPE_GAME, PACKAGE_SCOPE_LAUNCHER, RELEASE_STATE_NAME, ROOT_DIR_NAME
...
def main() -> None:
    ...
    try:
        ...
        smoke_config_path = _make_publish_config(
            ...
            backend_base_url=base_config.backendBaseUrl,
        ...
        launcher_update_v1 = _run_headless_update(
            workspace_root=workspace,
            target_root=target_root,
            launcher_dir=launcher_dir,
            launcher_exe=launcher_exe,
            server_base=base_config.backendBaseUrl,
            channel=channel,
            scope=PACKAGE_SCOPE_LAUNCHER,
        )
        game_update_v1 = _run_headless_update(
            workspace_root=workspace,
            target_root=target_root,
            launcher_dir=launcher_dir,
            launcher_exe=launcher_exe,
        ...
        launcher_manifest_v2 = _fetch_json(f"{base_config.backendBaseUrl}/updates/{channel}/releases/{publish_v2['releaseId']}/launcher/release-manifest.json")
        game_manifest_v2 = _fetch_json(f"{base_config.backendBaseUrl}/updates/{channel}/releases/{publish_v2['releaseId']}/game/release-manifest.json")
        _assert(launcher_obsolete_rel in (launcher_manifest_v2.get("deleteList") or []), "v2 launcher deleteList 未包含 obsolete.txt")
        _assert(game_obsolete_rel in (game_manifest_v2.get("deleteList") or []), "v2 game deleteList 未包含 obsolete.txt")

        launcher_args_file.unlink(missing_ok=True)
        launcher_update_v2 = _run_headless_update(
            workspace_root=workspace,
            target_root=target_root,
            launcher_dir=launcher_dir,
            launcher_exe=launcher_exe,
        ...
        game_update_v2 = _run_headless_update(
            workspace_root=workspace,
            target_root=target_root,
            launcher_dir=launcher_dir,
            launcher_exe=launcher_exe,
        ...
        _assert(not (launcher_dir / launcher_obsolete_rel).exists(), "v2 launcher 旧文件未删除")
        _assert(not (target_root / game_obsolete_rel).exists(), "v2 game 旧文件未删除")
        ...
        latest_url = f"{base_config.backendBaseUrl}/api/update/v1/channels/{channel}/latest"
            ...
            json.dumps(
                {
                    ...
                    "updates": {
                        "launcherV1": launcher_update_v1,
                        "gameV1": game_update_v1,
                        "launcherV2": launcher_update_v2,
                        "gameV2": game_update_v2,
                    },
                    "deleteResults": {
                        "deleteV1": delete_v1,
                        "deleteV3": delete_v3,
