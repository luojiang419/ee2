...
def _looks_like_updater_root(path: Path) -> bool:
    has_publish_config = (
        (path / "config" / "publish.local.json").exists()
        or (path / "config" / "publish.example.json").exists()
    )
    return has_publish_config and (path / "src" / "ee2x_update_suite").exists()
...
def workspace_root() -> Path:
    ...
    raise RuntimeError("未找到更新器工作目录（缺少 config/publish.local.json / publish.example.json 或 src/ee2x_update_suite）。")


def default_publish_config_path() -> Path:
    root = workspace_root()
    local_config = root / "config" / "publish.local.json"
    if local_config.exists():
        return local_config
    return root / "config" / "publish.example.json"
