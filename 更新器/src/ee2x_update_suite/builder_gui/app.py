...
from ee2x_update_suite.publisher.service import publish_dual_release
from ee2x_update_suite.shared.constants import LAUNCHER_DIR_NAME, ROOT_DIR_NAME
from ee2x_update_suite.shared.manifest_builder import create_dual_release_bundle, suggest_release_notes
...
class BuilderWindow:
    def __init__(self, root: tk.Tk) -> None:
        ...
        default_config = self.workspace_root / "config" / "publish.local.json"
        if not default_config.exists():
            default_config = self.workspace_root / "config" / "publish.example.json"
    ...
    def _append_paths(self, paths: list[Path]) -> None:
        ...
        for path in paths:
            ...
            if game_root != resolved and game_root not in resolved.parents:
                ...
                return
            relative_path = relative_to_root(game_root, resolved)
            if relative_path == LAUNCHER_DIR_NAME or relative_path.startswith(f"{LAUNCHER_DIR_NAME}/"):
                messagebox.showerror("路径错误", "当前轻量更新链仅允许发布 game 内容。")
                return
    ...
    def _toggle_tree_selection(self) -> None:
        ...
        if path is None:
            return
        relative_path = relative_to_root(self._game_root(), path)
        if relative_path == LAUNCHER_DIR_NAME or relative_path.startswith(f"{LAUNCHER_DIR_NAME}/"):
            messagebox.showerror("路径错误", "当前轻量更新链仅允许发布 game 内容。")
            return
    ...
    def _build_release(self) -> None:
        ...
        if not notes:
            selected_relative_paths = [relative_to_root(self._game_root(), item) for item in self.selected_paths]
            delete_list = [line.strip() for line in self.delete_box.get("1.0", "end").splitlines() if line.strip()]
            notes = suggest_release_notes(selected_relative_paths, delete_list)
    ...
    def _publish_release(self) -> None:
        if self.release_dir is None:
            ...
            return
        try:
            result = publish_dual_release(self.release_dir, Path(self.config_var.get()).resolve())
        except Exception as exc:
            messagebox.showerror("推送失败", str(exc))
            return
        package_lines = []
        for scope, info in sorted(result.get("packages", {}).items()):
            package_lines.append(f"{scope}: {info['packageUrl']}")
        messagebox.showinfo(
            "推送完成",
            "\n".join(
                [
