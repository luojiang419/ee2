...
def main() -> None:
    parser = argparse.ArgumentParser(description="Deploy EE2X yuanhang launcher installer update backend")
    parser.add_argument("--host", default="115.231.35.105")
    parser.add_argument("--port", type=int, default=22)
    parser.add_argument("--username", default="root")
    parser.add_argument("--password", required=True)
    parser.add_argument("--remote-root", default="/opt/ee2/ee2x-new-update/launcher-backend")
    parser.add_argument("--service-name", default="ee2x-launcher-update-yh.service")
    parser.add_argument("--app-port", type=int, default=3015)
    parser.add_argument("--static-base-url", default="http://115.231.35.105:3015")
    args = parser.parse_args()

    archive_bytes = build_archive()
    remote_parent = posixpath.dirname(args.remote_root.rstrip("/"))
    remote_archive = "/tmp/ee2x_launcher_update_backend_yh.tar.gz"
    staging_root = posixpath.join(remote_parent, "ee2x_launcher_update_backend_yh_staging")
    backup_root = posixpath.join(remote_parent, "backups")
    backup_stamp = datetime.now(timezone.utc).strftime("%Y%m%d-%H%M%S")
    backup_dir = posixpath.join(backup_root, f"ee2x-launcher-update-yh-{backup_stamp}")
    ...
    try:
        ...
        default_env = "\n".join(
            [
                "EE2X_YH_LAUNCHER_UPDATE_HOST=0.0.0.0",
                f"EE2X_YH_LAUNCHER_UPDATE_PORT={args.app_port}",
                "EE2X_YH_LAUNCHER_UPDATE_ADMIN_USERNAME=ee2x",
                "EE2X_YH_LAUNCHER_UPDATE_ADMIN_PASSWORD=ee2x",
                f"EE2X_YH_LAUNCHER_UPDATE_STATIC_BASE_URL={args.static_base_url}",
                f"EE2X_YH_LAUNCHER_UPDATE_STORAGE_ROOT={args.remote_root}/storage",
                f"EE2X_YH_LAUNCHER_UPDATE_DB_PATH={args.remote_root}/db/launcher_update.sqlite3",
                "",
            ]
        )
        ...
        try:
            ...
            service_template = (LOCAL_ROOT / "systemd" / "ee2x-launcher-update-yh.service").read_text(
                encoding="utf-8"
            )
            rendered_service = (
