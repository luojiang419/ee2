...
class WorkspaceLocator {
  static File _resolvePublishConfigPath(Directory updaterDir) {
    final localConfig = File(p.join(updaterDir.path, 'config', 'publish.local.json'));
    if (localConfig.existsSync()) {
      return localConfig;
    }
    return File(p.join(updaterDir.path, 'config', 'publish.example.json'));
  }
        ...
        if (visited.add(key)) {
          ...
          if (updaterDir.existsSync() && gameRoot.existsSync()) {
            return WorkspaceContext(
              ...
              gameRoot: gameRoot,
              publishConfigPath: _resolvePublishConfigPath(updaterDir),
              preferencePath: File(
...
class _Ee2xPublishToolAppState extends State<Ee2xPublishToolApp> {
  ...
  bool _pathCanBeSelected(String relativePath) {
    ...
    return node != null && !_isProtectedLauncherRelativePath(relativePath);
  }
  ...
  void _applyLauncherSafePreset() {
    _setActionMessage(
      '启动器文件可通过文件树手动勾选。受保护的运行时目录（Config/Logs/userdata等）会被自动排除。',
  ...
  Future<PublishBackendRuntimeConfig> _loadPublishRuntimeConfig() async {
    ...
    if (!workspace.publishConfigPath.existsSync()) {
      throw StateError('发布配置不存在：${workspace.publishConfigPath.path}');
    }
    final payload =
        jsonDecode(
    ...
    return PublishBackendRuntimeConfig(
      ...
      adminUsername: (payload['adminUsername'] as String? ?? 'ee2x').trim(),
      adminPassword: (payload['adminPassword'] as String? ?? 'ee2x').trim(),
  ...
  Future<Map<String, dynamic>> _uploadReleaseDirWithProgress({
    ...
    try {
      ...
      } else if (runtimeConfig.adminUsername.isNotEmpty) {
        final token = base64Encode(
          utf8.encode(
            '${runtimeConfig.adminUsername}:${runtimeConfig.adminPassword}',
          ),
        );
        clientRequest.headers.set(
          HttpHeaders.authorizationHeader,
  ...
  Future<void> _saveBundleFromMainForm() async {
    ...
    try {
      ...
      if (preparedGameCount <= 0) {
        throw StateError('本地打包完成后没有可导出文件，请确认已选择游戏文件。');
      }
      ...
      final exportSummaryMessage =
          '本次发布将按当前勾选的游戏文件生成更新包。\n'
          '未勾选文件不会自动进入包，显式删除项会按 manifest 下发。';
        ...
        children: [
          ...
          Wrap(
            ...
            children: [
              ...
              _statusChip('发布范围', _selectionSummary.launcherEntries.isNotEmpty ? 'Game + Launcher' : 'Game'),
              _statusChip('Game', '${_selectionSummary.gameEntries.length} 项'),
              if (_selectionSummary.launcherEntries.isNotEmpty)
                _statusChip('Launcher', '${_selectionSummary.launcherEntries.length} 项'),
            ],
          ),
        ],
      ),
        ...
        children: [
          ...
          if (_catalogTab == CatalogPanelTab.browse) ...[
            ...
            Align(
              ...
              child: Text(
                '当前发布范围为游戏根目录下的任意 game 文件；启动器目录保持只读。',
                style: TextStyle(
                  color: _palette.secondaryText,
                  fontSize: 12.5,
  ...
  Future<void> _showServerVersionDetails(ReleaseHistoryEntry entry) async {
    ...
    await showDialog<void>(
      ...
      builder: (dialogContext) {
        ...
        final dialogWidth = (mediaSize.width * 0.84).clamp(360.0, 960.0);
        final dialogHeight = (mediaSize.height * 0.84).clamp(420.0, 760.0);
        final dialogScale =
            ((dialogWidth / 960.0) * 0.55 + (dialogHeight / 760.0) * 0.45)
                .clamp(0.88, 1.08);
        final detailCardWidth = dialogWidth >= 760
            ? 250.0
            : (dialogWidth - 92).clamp(220.0, 320.0);

        return MediaQuery(
          data: MediaQuery.of(
            dialogContext,
          ).copyWith(textScaler: TextScaler.linear(dialogScale)),
          child: Dialog(
            backgroundColor: _palette.panel.withValues(alpha: 0.98),
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            Wrap(
                              ...
                              children: [
                                SizedBox(
                                  width: detailCardWidth.toDouble(),
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          _inlineBadge(
            favorite.scope == PackageScope.launcher ? 'Locked' : 'Game',
  ...
  Widget _buildSummaryPanel() {
    final summary = _selectionSummary;
    final selectedGameCount = summary.gameEntries.length;
    return GlassPanel(
      palette: _palette,
      ...
      child: Column(
        ...
        children: [
          ...
          Row(
            children: [
              Expanded(
                child: _summaryCard(
                  '发布范围',
                  _selectionSummary.launcherEntries.isNotEmpty ? 'Game + Launcher' : 'Game',
                  _palette.accentSoft,
                ),
              ),
              const SizedBox(width: 10),
          ...
          Text(
            '当前将按勾选的 game 路径打包发布；如需发布多个根目录文件，请直接一并勾选。',
            style: TextStyle(
              color: _palette.secondaryText,
              fontSize: 12.4,
              height: 1.5,
  ...
  Widget _buildTreeTile(VisibleTreeNode visibleNode) {
    ...
    final selectionDisabled = _isProtectedLauncherRelativePath(node.relativePath);
      ...
      child: Row(
        children: [
          ...
          Checkbox(
            tristate: true,
            value: checkboxValue,
            onChanged: selectionDisabled
                ? null
                : (_) => _toggleSelection(node.relativePath, nextValue),
          ),
          Icon(
            node.isDirectory
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 6),
          Text(
            '游戏文件 ${result.gameFileCount} 项',
            style: TextStyle(color: _palette.secondaryText, height: 1.45),
          ),
          const SizedBox(height: 6),
          Text(
            '客户端行为: 启动器内直接完成游戏内容同步',
            style: TextStyle(color: _palette.secondaryText, height: 1.45),
          ),
          const SizedBox(height: 6),
          if (result.gameDeletedCount > 0) ...[
            Text(
              '显式删除项: 游戏 ${result.gameDeletedCount} 项',
              style: TextStyle(color: _palette.secondaryText, height: 1.45),
            ),
            const SizedBox(height: 6),
          ] else ...[
