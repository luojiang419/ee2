...
function loadConfig(){
  ...
  try {
    const releaseStatePath = path.join(getAppRoot(), 'update', 'runtime', 'release-state.json')
    if (fs.existsSync(releaseStatePath)) {
      const releaseState = JSON.parse(fs.readFileSync(releaseStatePath, 'utf-8'))
      if (releaseState && releaseState.version) cfg.localVersion = releaseState.version
    }
  } catch (e) {
...
async function fetchStaticLatestDescriptor(cfg, preferredBaseUrl){
  ...
  for (const base of candidates){
    ...
    try {
      ...
      if (data && data.version && data.packageUrl) {
        return {
          base,
          channel,
          latest: {
...
ipcMain.handle('update:diff', async (evt, baseUrl) => {
  ...
  try {
    const { latest } = await fetchStaticLatestDescriptor(cfg, baseUrl)
    const localVersion = String(cfg.localVersion || '').trim()
...
ipcMain.handle('launcher:runUpdater', async (evt, latest) => {
  ...
  // 读取当前配置，获取服务器地址与根目录
  const cfg = loadConfig()
  const gameRoot = resolveGameBase(cfg)
  const repoRoot = path.dirname(gameRoot)
  const launcherDir = getAppRoot()
  const launcherExe = findLauncherExecutablePath()
  const channel = getStaticUpdateChannel(cfg)
  let serverBase = cfg.serverUrl || 'http://localhost:3003'
  try {
    const { base } = await fetchStaticLatestDescriptor(cfg, cfg.serverUrl)
    serverBase = base
  } catch (e) {
    log(`未能从静态清单自动确认服务器地址，使用配置值: ${e}`)
  }
  const args = [
    '--root', gameRoot,
  ...
  let launchCmd = ''
  let launchArgs = []
  let launchCwd = launcherDir
  const exeCandidates = [
    path.join(getAppRoot(), 'update', 'ee2x-up.exe'),            // 发布目录标准位置
    ...
    path.join(repoRoot, '更新器', 'dist', 'ee2x-up.exe')         // 开发目录打包产物
  ]
  for (const candidate of exeCandidates) {
    if (fs.existsSync(candidate)) {
      launchCmd = candidate
      launchArgs = args
      launchCwd = path.dirname(candidate)
      log(`找到更新程序: ${candidate}`)
      break
    }
  }
  if (!launchCmd) {
    const sourceEntry = path.join(repoRoot, '更新器', 'src', 'ee2x_update_suite', 'updater_gui', '__main__.py')
    if (fs.existsSync(sourceEntry)) {
      launchCmd = process.platform === 'win32' ? 'py' : 'python3'
      launchArgs = process.platform === 'win32' ? ['-3', sourceEntry, ...args] : [sourceEntry, ...args]
      launchCwd = repoRoot
      log(`使用源码模式启动更新器: ${sourceEntry}`)
    }
  }
  if (!launchCmd) {
    log(`未找到更新程序，尝试的路径: ${exeCandidates.join(', ')}`)
    return { ok: false, error: '未找到新的更新器可执行文件或源码入口' }
  }

  log(`启动更新器，服务器地址: ${serverBase}, 频道: ${channel}, 根目录: ${gameRoot}`)

  return new Promise((resolve) => {
    try {
      // 启动更新程序
      const child = require('child_process').spawn(launchCmd, launchArgs, {
        cwd: launchCwd,
  ...
  try {
    ...
    return {
      ...
      manifest: {
        ...
        notes: latest.notes || '',
        download_url: latest.packageUrl || '',
        force_update: false,
        file_size: latest.packageSize || 0,
        file_hash: latest.packageSha256 || '',
        created_at: latest.publishedAt || '',
        manifest_url: latest.manifestUrl || ''
      }
    }
  } catch (e) {
    log(`update2 manifest static failed: ${e}`)
  }
...
ipcMain.handle('getCurrentVersion', async () => {
  try {
    const releaseStatePath = path.join(getAppRoot(), 'update', 'runtime', 'release-state.json')
    if (fs.existsSync(releaseStatePath)) {
      const stateData = JSON.parse(fs.readFileSync(releaseStatePath, 'utf8'))
      if (stateData && stateData.version) return stateData.version
    }
